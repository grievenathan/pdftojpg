...
import { formatFileSize } from "@/lib/convert";
...
export interface FileItem {
  ...
  pages?: { pageNumber: number; blob: Blob }[];
  totalSize?: number;
  error?: string;
}
...
export default function FileCard({ item, onDownload, onRemove }: FileCardProps) {
  return (
    <div className="group flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-black/[0.03]">
      {/* Icon */}
      <div className="relative flex h-9 w-9 flex-shrink-0 items-center justify-center overflow-hidden rounded-md border border-border bg-red-50">
      ...
      <div className="flex-1 min-w-0">
        <p className="truncate text-sm text-gray-900">{item.file.name}</p>
        <p className="text-xs text-muted">
          {formatFileSize(item.file.size)}
          {item.status === "converting" && item.progress && (
            <span> &middot; Page {item.progress.done}/{item.progress.total}</span>
          )}
          {item.status === "done" && item.pages && (
            <span> &rarr; {item.pages.length} {item.pages.length === 1 ? "image" : "images"}{item.totalSize ? ` (${formatFileSize(item.totalSize)})` : ""}</span>
          )}
        </p>
      </div>
